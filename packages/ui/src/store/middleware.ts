...
localStorageMiddleware.startListening({
  ...
  effect: (action) => {
    if (typeof window !== 'undefined') {
      ...
      if (!isFromExternal) {
        console.log('💾 Salvando no localStorage e notificando outras apps:', action.payload);
        localStorage.setItem('redux-user-state', JSON.stringify(action.payload));

        window.dispatchEvent(new CustomEvent('redux-user-changed', {
          detail: action.payload
        }));
      } else {
        console.log('🔄 Mudança veio de sincronização externa, não disparando evento');
      }
    }
  },
});
